...
FlowRouter.route('/:connection/:database/:collection/commands', {
  name: 'Commands',
  action(params) {
    BlazeLayout.render("DefaultLayout", {
      template: 'Commands',
  ...
  getRouteParameters = () => {
    ...
    if(connectionSlug) {
      ...
      if(parameters.connection && databaseName) {
        parameters.database = Databases.findOne({name: databaseName, connection_id: parameters.connection._id});

        if(parameters.database && collectionName) {
          parameters.collection = Collections.findOne({name: collectionName, database_id: parameters.database._id});
        }
      }
    }
