...
connectDatabase = function(databaseId) {
  ...
  var t = wrappedConnect(connectionUrl);
  return t
};
...
getFilterRoute = function(filterId) {
  let t = FlowRouter.current();
  FlowRouter.go('/' + t.params.connection + '/' + t.params.database + '/' + t.params.collection + (filterId ? ('/' + filterId) : ''));
}
